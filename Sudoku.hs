...
-- 2. Apply Constraint Propagation (does not use Tony's type signature for Propagate):
...
-- Implement collapse
collapse = cp . map cp
-- Solving: main purpose is to convert between types: Sudoku, Matrix and String.
...
-- converts a matrix to string (typical Top-down)
...
search m
    ...
    | otherwise = [s | m' <- expandFirst m, s <- search (fixpoint prune_level_2 m')]
                                         -- change the number 1,2,3 to control different levels
...
-- that single position by guessing.
-- Obviously, if the num of choice equals to min_choice(>1), it will have least redundancy.
-- expandFirst filters the best position to fill first according to the number of possibilities,
-- which is exactly what we want in the extension 1.
...
expandFirst mc  = [rows1 ++ [row1 ++ [c] : row2] ++ rows2 | c <- cs]
    where
        ...
        best cs           = (length cs == n)
        n                 = min_choice mc
        min_choice         = minimum . filter(>1) . concat . map(map length)
-- Extension 2 : Constraint Propagation to prune the huge search space
...
-- Quickcheck for solve
prop_solve :: String -> Bool
prop_solve s
    | solution_list == [] = True
    | otherwise           = isSolutionOf solution_list s
    where solution_list = solve s

isSolutionOf :: [String] -> String -> Bool
solution_list `isSolutionOf` str = solution_list == solve str
-- all (==True) [ xs  solve str | xs <- solution_list]

-- run with fewerCheck if you do not like to wait
